// imageGen.js — canvas اختياري: إذا فشل التثبيت يعمل التطبيق بدونه
  ...
  try {
    ...
    console.warn("[imageGen] canvas غير متاح — profile/welcome cards معطلة:", e.message);
  }
  ...
  async function generateWelcomeCard({ name, avatarURL }) {
    if (!canvasAvailable) throw new Error("canvas غير مثبت على هذا الخادم");
  ...
  async function generateProfileCard({ name, exp, money, level }) {
    ...
    ctx.fillStyle = "#00d2ff";
    ctx.beginPath();
    ctx.roundRect(barX, barY, barW * (progress / expMax), barH, 12);
    ctx.fill();
    ctx.fillStyle = "#fff";
